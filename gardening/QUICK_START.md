# ⚡ Quick Start — Greenshadow Garden in 5 Minutes

## You're here because:
- You need to know what to plant RIGHT NOW
- You want a quick garden tour
- You're new and don't want to read 30 pages

---

## 🌍 The Basics

**Zone:** 7b | **Location:** Fuquay-Varina, NC | **Spring Frost:** April 15 (safe date)

**Garden Philosophy:** Sustainable. Self-seeding. Witchy. Herbs matter. Let nature do the work.

---

## 📍 What Are the Gardens?

| Garden | What | Why |
|--------|------|-----|
| **Wildwood** | Fenced veggie area: 8 beds, corn circle, herbs, melons | Food production |
| **Bloomveil** | Colorful flower garden (L-shaped) | Pollinators + beauty + cut flowers |
| **Edgeworth Corner** | Grid pollinator garden (sunny + shade) | More pollinators, low maintenance |
| **Wayward Meadow** | Back corner wildflower meadow | Prairie plants, native bees |
| **Fernwood** | Shade garden (north side) | Ferns, hostas, rest area |
| **Swingnook** | Pergola retreat with swing | Hang out + bonfire |
| **Small Gardens** | Cat garden, berry beds, porch pots, front | Extras scattered around |

---

## 📅 What to Do Right Now (March 9, 2026)

**This week:**
- [ ] Start seeds indoors: tomatoes, peppers, eggplant (6-8 weeks before April 15)
- [ ] Check soil moisture — water if dry
- [ ] Plan herb spiral placement
- [ ] Read [Planting_Calendar_NC.md](guides/Planting_Calendar_NC.md) for April dates

**Next week:**
- [ ] Start cover crop (buckwheat) in Corn Circle
- [ ] Harden off any early seedlings
- [ ] Begin direct sowing: cool-season crops (lettuce, peas, spinach)

**By April 15:**
- [ ] All danger of frost has passed — transplant warm-season stuff (tomatoes, peppers, basil, etc.)

See [March_Schedule.md](guides/March_Schedule.md) for the full day-by-day plan.

---

## 🌱 What You Already Have

You own **228 seeds and plants** organized by:
- **Vegetables** — tomatoes, peppers, melons, squash, beans, corn, greens
- **Herbs** — basil, rosemary, sage, thyme, mint, lavender, chamomile, calendula, lemon balm, and more
- **Flowers** — zinnias, cosmos, sunflowers, dahlias, coneflowers, marigolds, nasturtiums
- **Natives** — milkweed, black-eyed susans, bee balm, coneflower (for pollinators)

**Full list:** [Master_Seed_and_Plant_List.md](inventory/Master_Seed_and_Plant_List.md)

---

## 🔍 How to Use This Repo

### "I want to know what to plant in [garden]"
→ Go to `beds/[Garden_Name].md`

Examples:
- [beds/Wildwood.md](beds/Wildwood.md) — veggie beds layout
- [beds/Bloomveil.md](beds/Bloomveil.md) — flower garden zones
- [beds/Small_Gardens.md](beds/Small_Gardens.md) — everything else

### "What grows in my zone now?"
→ [guides/Planting_Calendar_NC.md](guides/Planting_Calendar_NC.md)

### "How do I plant seeds/transplants?"
→ [guides/Planting_How_To.md](guides/Planting_How_To.md)

### "I have bugs/disease/problem"
→ [guides/Garden_Care_and_Troubleshooting.md](guides/Garden_Care_and_Troubleshooting.md)

### "What's that herb for?"
→ [guides/Medicinal_Herb_Kit_Guide.md](guides/Medicinal_Herb_Kit_Guide.md)

### "What grows where?"
→ [inventory/Master_Seed_and_Plant_List.md](inventory/Master_Seed_and_Plant_List.md) or the garden-specific grids

---

## 📱 Subscribe to the Calendar

All your garden dates in one place. Add to your phone/Google Calendar:

**URL:** `https://raw.githubusercontent.com/joyrmac/GardeningonGreenshadow/main/gardening/Joy_Garden_2026.ics`

[Full instructions here.](../README.md#subscribing-to-the-calendar)

---

## 🎯 Next Steps

1. **Today:** Read [Garden_Overview.md](Garden_Overview.md) (the philosophy part makes sense of everything)
2. **This week:** Check [March_Schedule.md](guides/March_Schedule.md) and start seeds
3. **By April 15:** Have seedlings hardened off and ready to plant
4. **Ongoing:** Reference [Planting_Calendar_NC.md](guides/Planting_Calendar_NC.md) for what's next

---

## 🔗 Key Files (Bookmark These)

- **What to plant when:** [Planting_Calendar_NC.md](guides/Planting_Calendar_NC.md)
- **This month's plan:** [March_Schedule.md](guides/March_Schedule.md)
- **How to do things:** [Planting_How_To.md](guides/Planting_How_To.md)
- **Troubleshoot problems:** [Garden_Care_and_Troubleshooting.md](guides/Garden_Care_and_Troubleshooting.md)
- **All your seeds:** [Master_Seed_and_Plant_List.md](inventory/Master_Seed_and_Plant_List.md)

---

**Want the full deep dive?** → [Garden_Overview.md](Garden_Overview.md)

**Got a question?** → Check the relevant file in `guides/` or `reference/`

**Ready to plan your space?** → Pick a garden in `beds/` and dive in.

---

*Last updated: March 9, 2026*
